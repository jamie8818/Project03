...
**✅ 引擎已接手完成（2026-07-10）**：儲存走 shop state 加欄位 `guestLines`（每人一句 ≤20 字）；worker/devApi POST 按鍵合併（`{...cur, ...body}`，舊客戶端不帶欄位＝保留現值，客戶端只寫自己的鍵）——**worker 有改，部署要 `npx wrangler deploy`**。UI：talk 模式點自己客人→編輯面板（預填現值、Enter/掛上去儲存、清空＝取消台詞）；點對方→彈對方台詞氣泡（沒設顯示「〈名字〉今天也有來喔」）。防雜訊三規則全做：💬 指示點（有台詞才亮）、氣泡點了才彈 4.2s 自動收、全域單氣泡（客人氣泡起→店長碎念隱藏；點店長/展示座→關客人氣泡）；氣泡錨頭上 max-width 180px。preview 全流程驗證含 server 兩鍵並存不互蓋。
...
**素材**：`public/cafe/cat/{sit,groom,roll}.png`，三張皆 RGBA、統一畫布 **80×68px**、著地線 **canvas 內 y=62**（bottom 留 6px 緩衝，不觸邊）、內容已水平置中於畫布（canvas 中心 x=40）。四張用**同一組全域縮放係數**（不是各自獨立 fit-to-canvas）處理，因為源圖（JJ 提供、`/Users/huangchengchieh/Desktop/q版人/粉圓/` 四張 UUID 檔）經量測本來就是同一批次同一原生尺度（sit/groom 兩張坐姿 crop 高度 812px/806px 幾乎相等，證實批次原生比例一致），維持姿勢間的真實相對大小差異（坐姿較高窄、仰躺較寬扁，符合直覺、不強拉同高）。
...
| `roll` | 仰躺露白肚、前腳彎抬胸前、後腳舉起露肉球、尾捲右下（源圖冷調、PIL 調暖定稿；JJ 確認三姿勢、同源另一張已刪） | 50×41 | x15–64, y21–61 |
...
| ④ | 門口展示櫃旁 | x≈350, y=370 | `roll` | 原提案 x≈330 實測會疊到門本體（`base-fg` 門框），已挪至展示櫃（E8 x≈352）左側清空區；進門先看到一隻攤平的貓，反差可愛；繪序同②③ |

四點位已各配一種姿勢、彼此不重複，`spotIndex = floor(Date.now() / 600000) % spots.length`（`spots.length=4`）即可直接映射到上表順序。若引擎想擴到 5–6 點位，三姿勢可跨點位重複使用（貓在不同地方擺同姿勢很正常）。注意①與④同為 roll——JJ 定稿三姿勢，接受重複。
...
**✅ Tier B 引擎已接手完成（2026-07-10，與 E18/E19 同批）**：`UserState.meta` 輕量計數袋＋工具（bumpMeta/addDailyAmount 跨日歸零/bumpDailyStreak 同日冪等）。9 條全上：查水表/頭號粉絲（同計數 pandaClicks 雙門檻 50/100，點店長換句計；不計自動輪播避免每 8s 寫存檔）、夜貓/時差（completeSession 注入台北時制 hour 記旗標，可測試）、一擲千金（購買+轉蛋單日花費累計）、扭蛋沼（單日轉數）、極簡主義（今日完成且開店看時 layout 空→連續日）、雙人全勤（App 層兩人同日完成→連續日）、貓奴認證（改綁 E19 好感 100，照單）。單元測試 +2（工具函式/時段旗標）；preview 實測 pandaClicks 49→50 解鎖查水表 toast+20🪙 全管線。
...
### 摸頭互動（E18 加碼，JJ 需求 2026-07-10 第二批）

**素材**：`public/cafe/cat/` 新增 7 檔，工法同上（PIL 區域手術＋逐像素 diff 驗證改動矩形外＝0、原檔備份 scratchpad、numpy 孤立洞/孤立浮島檢測皆 0），A 幀原檔與上述三張 `*_b.png` 全數未動。

- `hand_pet.png`：全新獨立圖（非既有幀改造），24×20px 透明畫布，畫風比照 `public/cafe/guests/jj.png` 手部（暖膚色系、深描邊 #1B120A）。簡化手掌自畫布頂端伸入（頂邊刻意不描邊，視覺上手臂延伸至畫布外），四指分兩段內彎（straight→bend→tip 三段式，非直條硬切），模擬微彎向下摸的手勢；掌心/掌緣各留 1px 高光與陰影做立體感。
- `sit_pet.png`／`groom_pet.png`／`roll_pet.png`（爽臉幀）：眼睛改「淺色眼皮底＋深色 ^ 弧線」疊層（純暗色描邊在 80×68 小尺寸下對比太弱，已依 codex 第一輪意見加寬加粗），弧線 apex+雙腳共 5px 寬；頰側加 2px 腮紅（半透明疊色，不完全覆蓋原毛色紋理）。改動矩形：`sit` (36,25)-(54,32) 16px／`groom` (29,29)-(37,34) 8px／`roll` (20,28)-(31,36) 14px，**矩形外 diff pixels = 0**。
- `sit_dodge.png`／`groom_dodge.png`／`roll_dodge.png`（躲頭幀）：頭部（含雙耳）整塊水平平移 3px（`sit`/`roll` 向左、`groom` 向右，避開各自既有低頭朝向）＋耳尖水平切平 2px（模擬耳朵壓平／被拒與生氣共用，生氣時引擎另加 CSS 💢）。`roll` 因頭部與胸口毛皮連續無自然分界，平移矩形右緣切過連續毛皮會產生破洞（codex 第一輪抓到的「白色直角缺口」），已改為僅在頭部與軀幹融合的下半段用原圖對應像素局部補洞、耳尖範圍不補（避免像 `sit`/`groom` 耳朵那樣補出殘影）。改動矩形：`sit` (16,9)-(63,38) 538px／`groom` (19,14)-(52,37) 537px／`roll` (9,18)-(43,45) 574px，**矩形外 diff pixels = 0**，isolated_holes／isolated_islands 皆 = 0。

**QC**：7 件素材（3 組 A/pet/dodge 三連對照圖＋hand_pet 單張放大）已過 codex 前景審查，共兩輪：
- 第一輪意見：①pet 幀瞇眼弧對比太弱（暗色疊暗色毛皮不清楚）②`roll_dodge` 頭部與軀幹交界白色直角破洞③`hand_pet` 手指過直、彎曲感不足。三點皆已修正。
- 第二輪覆審結論：**可定案**，三項修正皆確認有效，無殘留問題。

**引擎待辦（摸頭互動邏輯）**：
1. 點擊粉圓 → 疊圖 `hand_pet.png` 貼於貓頭正上方，播「拍 3 下」動畫：`translateY` 在 0px/±3px 間來回，共 ~1.2s（比照 E9/E15 既有雙圖疊層＋CSS transform 手法，非 sprite-sheet）。
2. 同時貓的 A 幀切到對應 `*_pet.png`，維持 1.2s（跟拍手動畫同步收尾）後切回 A 幀。
3. CSS 小 ❤（emoji 或簡易圖形皆可，引擎擇一）從貓頭頂位置往上飄升 20px＋淡出，跟拍手動畫同時觸發。
4. **結果分支交給 E19 好感擲骰決定**：擲骰通過 → 上述 pet＋❤ 流程；擲骰失敗（含賭氣期間必拒）→ 改切對應 `*_dodge.png` 0.8s（無 ❤），貓頭撇開，拍手動畫可省略或縮短為手撲空的感覺。
5. **取代 E15「可選加分」的『點貓冒…／ニャ』設計**：E15 該段（點擊反饋頭上冒「…」2 秒淡出）作廢，改用本節摸頭互動流程；E15 原文保留不改，僅在此註記取代關係。

## E19. 粉圓好感度系統（JJ 需求 2026-07-10）

**概念**：呼應 E18 摸頭互動，粉圓對亞軒／JJ 各自獨立的好感值 0-100，決定每次摸頭是否成功（pet／dodge 分支）、並驅動長期關係階級顯示。以下數值**全做引擎常數**（不要寫死魔數），方便日後調參。無美術素材待辦，純資料/邏輯。

**資料模型**（存入 UserState 同步，比照 E14 `guestLines`／既有欄位擴充慣例；舊客戶端不帶欄位＝視同未初始化，引擎需給預設值）：

| 欄位 | 說明 |
|---|---|
| `catAffection.{亞軒,JJ}` | 0-100 整數，兩人各自獨立 |
| `catAffection.{...}.lastPetAt` | 上次「有效摸」時間戳（冷卻判定用） |
| `catAffection.{...}.recentPets` | 60s 滑動視窗內點擊次數（連摸懲罰判定用，timestamp 陣列或計數＋視窗起點皆可） |
| `catAffection.{...}.sulkUntil` | 賭氣結束時間戳（賭氣期間必拒） |

**起始值**：亞軒 15／JJ 5（**JJ 待覆核**——目前先按此寫，若覆核後想改成跟亞軒同值，直接改常數即可，不影響架構）。

**常數與規則**：

| 常數 | 值 | 說明 |
|---|---|---|
| `AFFECTION_COOLDOWN` | 1800s（30 分） | 冷卻內的點擊不算「有效摸」——不擲骰、不加分、不更新 `lastPetAt`；是否仍播拍手動畫給輕量回饋，引擎自行斟酌 |
| 接受率公式 | `30% + 好感×0.6%` | 好感 0 時 30%、好感 100 時 90% |
| 接受後好感增量 | `+2`（cap 100） | |
| 拒絕 | 不扣分、冷卻不重置（意即可以馬上再試手氣，但仍受連摸懲罰視窗限制） | |
| 連摸懲罰 | 60s 內第 3 次點擊（含）→ 生氣 | dodge 幀＋CSS 💢、好感 `-3`（floor 0）、設 `sulkUntil = now + 300s` |
| 賭氣期間 | `sulkUntil` 未到期前必拒（不擲骰，直接 dodge） | |

**判定流程建議**（引擎可依實作習慣調整，邏輯需等價）：
1. 點擊 → 若在 `sulkUntil` 內 → 直接 dodge，結束。
2. 否則檢查 60s 視窗內點擊數，若這是第 3 次（含）以上 → 生氣分支（dodge＋💢＋好感−3 floor 0＋設定 `sulkUntil`），結束。
3. 否則若距 `lastPetAt` < `AFFECTION_COOLDOWN` → 「無效摸」，不擲骰不加分（是否播拍手動畫見上）。
4. 否則為「有效摸」→ 依上表公式擲骰 → 通過走 E18 的 pet＋❤ 流程＋好感+2＋更新 `lastPetAt`；不通過走 dodge 流程（不扣分、不更新 `lastPetAt`，代表馬上可以再摸一次試手氣，只要沒撞到連摸懲罰）。
...
摸頭成功後，貓頭頂另外浮現 2s 迷你愛心進度條（5 格 CSS，依當前好感/100 填格）——這是「這一摸」的即時反饋，跟階級圖示（常駐/半常駐小提示）用途不同，引擎可視覺上整合但語意分開。

**成就連動（呼應 E16）**：`cat-person`（貓奴認證）建議改綁「好感達 100」，取代原「點擊粉圓 20 次」門檻——有連摸懲罰／賭氣機制後，狂點刷次數會直接觸發生氣，20 次點擊不等於 20 次有效互動，跟「認證貓奴」培養感情的立意不符；改綁好感值更能反映真實培養結果。

**存檔**：`lastPetAt`／`sulkUntil` 兩個 timestamp 跟好感值一起存檔同步，確保跨裝置/重新整理後冷卻與賭氣狀態不會被重置成「隨時可摸」。

**✅ 引擎已接手完成（2026-07-10，E18＋E19 同批）**：
