...
## E22. 每日登入週禮物：熊貓店長無厘頭家具 ×6（2026-07-11 引擎開單）

**背景**：新上線「每日登入系統」——每天首登跳一封熊貓店長情勒信（+15🪙），累積登入第 1 天送第 1 件熊貓家具、之後每滿 7 天送下一件（第 8/15/22/29/36 天），六件送完後家具日改發轉蛋基金 80🪙。

**引擎已接完、素材到貨即自動生效**：id 已寫死在 `src/data/panda-mail.ts`；玩家賺到但素材還沒進 catalog 時先「掛帳」，你把 id 加進 `cafe-catalog.json`＋出圖重跑產生器，玩家下次進店自動入庫（照順序補發，**id 千萬照抄**）。徽章不用做——`streak-7.png`/`streak-30.png` 已在庫。

| # | id | 名稱 | 無厘頭概念（flavor 自由發揮，爛一點沒關係） |
|---|---|---|---|
| 1 | `panda_daruma` | 熊貓不倒翁 | 推倒會自己爬起來，號稱學習意志的化身 |
| 2 | `bamboo_grinder` | 竹筒磨豆機 | 店長宣稱竹香拿鐵的祕密兵器（沒人點過竹香拿鐵） |
| 3 | `panda_neon_sign` | 熊貓霓虹燈 | 熊貓舉牌「本日も営業中」，晚上比老闆勤勞（可考慮 anim loop 呼吸） |
| 4 | `panda_dakimakura` | 店長等身抱枕 | 店長本人監修，眼神一模一樣，抱著有罪惡感 |
| 5 | `bamboo_parasol_table` | 竹傘圓桌 | 傘頂躺一隻迷你熊貓，趕不走，桌面建議 surface: true |
| 6 | `panda_postbox` | 熊貓郵筒 | 每日情勒信的寄件源頭，頭上頂著信封 |
